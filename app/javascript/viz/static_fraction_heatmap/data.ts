...
export function plotDataloaded(_data){
  ...
  let methodlist = _data.map(d=>d["method"]) //conces的
  this.data.methodlist = methodlist
  this.data.dismethodlist = Array.from(new Set(this.data.methodlist))

  let sampleList = []
  for(let i = 0;i < _data.length;i++){
    sampleList.push(_data[i][_data.columns[0]]+"_"+_data[i][_data.columns[1]])
  }
  ...
  // console.log("中间左边: boxdata:",newData)
  ...
  let range1 = testrange1 + 5/(10 ** (cout2+1)) + 0.1
  ...
  let valueRange = [range0.toFixed(1),range1.toFixed(1)]; //boxplot value range
  ...
  _data.forEach((row,k) => {
    ...
    sumMap[row[categoryKey]+"_"+methodlist[k]] = count;
    sum.push(count)
  });
  ...
  _data.forEach((d,n) => {
    //console.log("_data d",d)
    classifications.forEach((classification,k) => {
      //cell
      d[classification]=="NA"? d[classification]=0:null
      result[classification].push([d[_data.columns[0]]+"_"+methodlist[n], Math.abs(parseFloat(d[classification]))/sumMap[d[categoryKey]+"_"+methodlist[n]]]); //这里除以和
    });
  });
...
export function clinicalDataloaded(_data){
  ...
  for(let i = 0;i < addNames.length;i++){
    // console.log("addNames[i].split",addNames[i].split("_")[0])
    let index = clinicalSamples.indexOf(addNames[i].split("_")[0]) //对应sample name
    if(index != -1){ //如果匹配上了
...
export function mapColor(cell){
  ...
  let colorMap = {
      ...
      "NKcells":"#f45740",
      "NK_cells":"#f45740",
      "NK cell actived":"#f48777",
      "NK cells activated":"#f48777",
      "NK cells resting":"#f4878d",
      "NK cell resting":"#f4878d",
      "Plasmacytoid dendritic cell (pDC)":"#f47040",
      "Plasma cells":"#f47040",
      "Plasma_cells":"#f47040",
      "Plasmablasts":"#f47040",
      "pDC cells":"#f47040",
      "pDCs":"#f47040",
      "pDC":"#f47040",
      "Cytotoxic cell":"#f46453",
      "Cytotoxic":"#f46453",
      "Cytotoxic cells":"#f46453",
      "Cytotoxic_cells":"#f46453",
      "Cytotoxic lymphocytes":"#f46453",
      "T cell":"#dd3c67",
      "T cells":"#dd3c67",
      "T cell CD4+":"#3271c2",
      "T_cell.CD4":"#3271c2",
